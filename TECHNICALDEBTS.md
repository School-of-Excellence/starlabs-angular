...
>
> ✅ **RE-ANCHORED to `production` 2026-06-04 (stream B done).** Angular `file:line` refs below re-verified against `production`. **Changed:** TD-010 — **5 of 6 routes are now guarded**, only `devtestmic` remains; TD-015 `:557→:584`; TD-016-history `:814→:815`. **Confirmed unchanged:** TD-006 `authguard.service.ts:186,188`. Data-driven debts + CF-repo (`development`) cites were always branch-independent.
...
FCM legacy server keys at `authguard.service.ts:186,188` (confirmed unchanged on `production` 2026-06-04) and the Zoom SDK key (`rjad2eLZSIKlamaIwi09tw`) — on `production` in `big/zoom-meeting`, `Scheduling/appointment-zoom-view`, `queue system/zoom-clientview` — ship in the public JS bundle. Rotate + move push-send server-side. A background task was spawned for this. **Still live on `production`.**
...
### TD-010 · Unguarded routes (SECURITY) — ✅ mostly fixed on `production`
**Re-checked on `production` 2026-06-04:** of the 6 originally-flagged routes, **5 are now guarded** — `view-participant-atc`, `evolutionwishlist`, `arenadesigninsights`, `create-workshop`, `workshopconfig/:id` all carry `canActivate` on `production`. **Only `devtestmic` (`app.routes.ts:322`) remains unguarded** — a dev/test mic screen; remove or guard it. (The original 6-route finding was against the stale `main`.) Evidence: `app.routes.ts` on `production`.
...
The remaining debt: the watson id is **not on the participant/profile record itself**, so screens that start from a profile (e.g. `journeyplan.component.ts:584` on `production`, `watsonParticipantSchedule()`) fall back to an **email** lookup (`Watson Participants where email == profile_data.email`) — case/whitespace/duplicate-sensitive, breaks on email change. Consider surfacing `watsonparticipantid` onto `profile_data` for a stable key. Evidence: `2026-06-03-watson-finance-and-shifted.md`, `watsonid_check.js`/`watsonid_check2.js`.
