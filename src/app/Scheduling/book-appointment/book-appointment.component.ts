...
export class BookAppointmentComponent implements OnInit {
  ...
  returnClient() {
    const profile = this.profileList.filter(e => e.name.toLowerCase().includes(this.filteredProfile.toLowerCase()));
  ...
  async getMyAppointment() {
    ...
    await getDocs(productQuery).then(async participantproducts => {
      ...
      var mapDeliverables = []
      this.deliverableCollection = collection(this.firestore, "deliverables")
      var deliveryQuery
      if (this.superRole) {
        deliveryQuery = query(this.deliverableCollection, where("profileid", "==", this.selectedUser), where("type", "==", "appointment"))
      }
      else {
        deliveryQuery = query(this.deliverableCollection, where("profileid", "==", this.selectedUser), where("type", "==", "appointment"), where("status", "==", "ready"))
      }
