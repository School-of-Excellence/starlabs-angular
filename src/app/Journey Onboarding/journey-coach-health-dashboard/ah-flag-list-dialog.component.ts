...
@Component({
  ...
  template: `
    <h2 mat-dialog-title class="afl-title">
      {{ data.title }}
      <span data-testid="afl-count" class="afl-count">{{ data.entries.length }}</span>
    </h2>
    <mat-dialog-content class="afl-content">
      <p *ngIf="!data.entries.length" class="afl-empty">No records in this window.</p>
      <button data-testid="afl-row" *ngFor="let e of data.entries" type="button" class="afl-row"
    ...
    <mat-dialog-actions align="end">
      <button data-testid="afl-close" mat-button (click)="close()">Close</button>
    </mat-dialog-actions>
  `,
  styles: [`
