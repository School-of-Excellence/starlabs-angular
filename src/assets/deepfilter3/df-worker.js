/**
 * df-worker.js — Web Worker for DeepFilterNet3 WASM processing
 * Runs WASM in a regular Worker context (not AudioWorklet) where SIMD works.
 * Receives audio frames via postMessage, returns denoised frames.
 *
 * Message protocol:
 *   IN  { type: 'init',    wasmBytes: ArrayBuffer, modelBytes: ArrayBuffer, suppressionLevel: number }
 *   IN  { type: 'process', inputFrame: Float32Array }
 *   OUT { type: 'ready',   frameLength: number }
 *   OUT { type: 'processed', outputFrame: Float32Array }
 *   OUT { type: 'error',   message: string }
 */

// Workers don't have `window` — the wasm-bindgen glue uses it as a global
// object fallback. Without this polyfill, accessing `window.window` throws
// ReferenceError which wasm-bindgen stores via __wbindgen_exn_store and then
// re-throws as a Rust panic → RuntimeError: unreachable.
if (typeof window === 'undefined') { var window = self; }
...
self.onmessage = async function(e) {
  ...
  if (msg.type === 'init') {
    try {
      ...
      // The dev/production server may serve .tar.gz with Content-Encoding: gzip,
      // causing the browser to auto-decompress it before JS sees it. WASM expects
      // gzip-compressed bytes — detect and re-compress if needed.
      let modelBytes = new Uint8Array(msg.modelBytes);
      const isGzip = modelBytes[0] === 0x1f && modelBytes[1] === 0x8b;
      if (!isGzip && typeof CompressionStream !== 'undefined') {
        const cs = new CompressionStream('gzip');
        const writer = cs.writable.getWriter();
        writer.write(modelBytes);
        writer.close();
        modelBytes = new Uint8Array(await new Response(cs.readable).arrayBuffer());
      }

      const handle      = df_create(modelBytes, msg.suppressionLevel ?? 80);
      const frameLength = df_get_frame_length(handle);

      dfModel = { handle, frameLength };
  ...
  if (msg.type === 'process' && dfModel) {
    try {
      const input     = new Float32Array(msg.inputFrame);
      const processed = df_process_frame(dfModel.handle, input);
      // Transfer the buffer back (zero-copy)
      self.postMessage({ type: 'processed', outputFrame: processed }, [processed.buffer]);
    } catch (err) {
      // On processing error, echo input so audio never cuts out
      self.postMessage({ type: 'processed', outputFrame: msg.inputFrame }, [msg.inputFrame.buffer]);
    }
  }
};
