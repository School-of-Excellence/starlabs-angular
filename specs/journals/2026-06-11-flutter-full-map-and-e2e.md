...
### Phase 3 — build milestones (in progress)
- **Foundation GREEN ✅** — render de-risk passed: seeded cohort user (participant90) real-login → Home renders (5-tab `bottomNav=true`) → queue card renders → real OS screenshot captured (637KB, stddev 0.14, non-blank). Fixed 3 real render-chain reads the Home does UNGUARDED: `queue generation` needs `queueenddate`/`queuestartdate`/`docid` (home.dart) + a `stageproperty` entry per stage (queueControl.dart:350) + `queue variation` doc so `queuestages` resolves (queueControl.dart:71 `indexOf==-1` → `[-1]` RangeError).
- **Suite authored** — via 2 dynamic workflows (16 Opus agents): 10 bucket seeders (`seed-<bucket>.cjs`, all run exit=0) + 10 bucket tests (`features/<bucket>_test.dart`, all `flutter analyze` clean) + the journey-flow seeder/test (`seed-journeyflow.cjs` + `journey_flow_test.dart`, onboarding→shell→delivery→events→content walk for user 170). Each bucket = anti-circular assertions on the catalog's `Writes` column; sim-blocked legs (camera/QR/CallKit/video/push/OpenAI) honestly drive-to-render. Workflow gotcha: schema-forced StructuredOutput made agents drop files mid-run → re-ran the unfinished buckets schema-free (write files + short text return).
- **Harness:** `run-flutter-test.cjs` (CAP-marker→`xcrun simctl io screenshot`, first-build-only xcconfig clear → fast incremental re-runs), `run-suite.cjs` (seed-all → 11 tests serial → RESULTS.md), `mobile-guards.cjs` (home crash-guards).
- **NOW:** full-suite baseline run in flight (seeds + 11 tests). Iterating per-bucket failures from `/tmp/flutterdrive-<bucket>.log` (first found: queue slot-booking RangeError → fixed via the queue-variation seed). Commits: `150f4c9`/`84ed561` (harness+4 tests), `dff87f1`/`4076cf7` (10 seeders+6 tests), `3d2e7e8`/`5f97521` (queue fix + JF test).

### Drive-by fix — full-screen self-move was a dead branch (queueStageDetail.dart)
- **Found:** `QueueStageDetail.actionButton()` set `type = "selfmovalble"` (typo) at `breakthroughs-flutter/lib/Delivery Queue/queueStageDetail.dart:73`, but the tap `switch` matches `case "selfmovable:"` (`:164`). So tapping **"Ready for Next Stage"** on the **full-screen Process Timeline** fell through to `default: print(type)` and **never called `appService.moveQueueStage(context)`** — the detail-screen self-move did nothing.
- **Why only the detail screen:** the Home-card path in `queueControl.dart` already used the correct `type = "selfmovable"` (`:178`) → `case "selfmovable"` (`:298`), so it worked. Bug was isolated to the full-screen detail.
- **Fix:** one char, `selfmovalble`→`selfmovable` (`:73`). grep confirms the misspelling was unique to that single line (zero other refs in `breakthroughs-flutter/`). `flutter analyze` clean — 6 issues, all **pre-existing** `info` lints (`intl` dep note `:12`, four `use_build_context_synchronously` `:126–156`, deprecated `withOpacity` `:433`), **none new, none at `:73`**. Committed `5402012` on `breakthroughs-flutter@development` (nested repo; **not pushed** — gated).
- **Relevance to Phase 3:** the **U4 Queue** bucket / `queue_test.dart` F9 self-move only exercises the path that was already green; the detail-screen leg of self-move now works too. (No test/behaviour changed elsewhere — scoped one-line correctness fix.)
