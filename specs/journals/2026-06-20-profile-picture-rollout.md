...
> NOTE: Sections A–D above were committed + merged to `production` on 2026-06-21 (commits `9c97f0e`, `ef86a77`, `45beda1`, `0cce457`; merge `e504ff3`). Section E below is the only currently-uncommitted change.

### E. POST-RELEASE FIX — uncommitted (2026-06-21): avatar-preview flicker on the Customer Support dashboard
5. **`customer-support-dashboard` ticket table — added `trackBy`** (`.html` `<tr *ngFor="let element of paginatedData; … trackBy: trackByTicket">` + `trackByTicket()` in `.ts`). The table is fed by a live `collectionSnapshots` listener that rebuilds `paginatedData` on every emit; with no trackBy the whole table — and every embedded `<app-profile-picture>` — was destroyed/recreated each emit. So an open avatar preview blinked out/in, and a click could land on the recreated cell (firing `messageIssue` → navigate to the ticket) instead of the avatar = the flicker. trackBy by ticket `id` keeps rows/components alive → preview opens stably (verified in-browser: overlay created once, 0 removals, no navigation; also removed the prior ~45s main-thread freeze on click). **Revert:** `git checkout -- "src/app/Customer Support/customer-support-dashboard/customer-support-dashboard.component.html" "src/app/Customer Support/customer-support-dashboard/customer-support-dashboard.component.ts"`.
6. **Shared `ProfilePictureComponent` → `ChangeDetectionStrategy.OnPush`** (`profile-picture.component.ts`) + `cdr.markForCheck()` in `onImgError` (and the body-overlay open/close in item 7 needs no markForCheck). OnPush limits this leaf's CD to its own state so a heavy host's per-mouse-move/snapshot CD no longer re-checks it. Verified avatars still load (dashboard 20/20, profilelist 25/25). **Shared component — affects every screen** (safe: self-contained leaf, primitive inputs).
7. **THE actual preview fix — mount the enlarge preview on `document.body`** (`profile-picture.component.ts` `openPreview`/`closePreview` build/remove a body-level `<div>` overlay; `ngOnDestroy` cleans up; removed the in-template `.preview-backdrop` block from `profile-picture.component.html`). Operator reported on the Customer Support table: clicking an avatar showed the enlarged image *near the bottom/over the table, not centered, no dim*, and it *flickered* on mouse-move. Root cause: the preview was rendered **inside the table row**, so the dashboard's constant re-render/churn (and a ~45s freeze on click) mis-positioned and destroyed/recreated it. Mounting the overlay directly on `document.body` (inline styles, since component-scoped CSS won't apply off-tree) fully decouples it: always a single, viewport-centered, dimmed overlay regardless of the host. Verified in-browser: overlay parent = BODY, image 320×320 centered (837/430 vs viewport 840/448), **1 overlay, unchanged after 4 mouse-moves** (no flicker), closes on backdrop/✕ click; works on profilelist too. **Shared component — affects every screen.** **Revert (items 6+7 together):** `git checkout -- "src/app/ProfilePicture/profile-picture/profile-picture.component.ts" "src/app/ProfilePicture/profile-picture/profile-picture.component.html"`.
