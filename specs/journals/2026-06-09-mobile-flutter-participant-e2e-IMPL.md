...
- **Pending:** the full 9-variation walk is running (`npx playwright test --config=playwright.mobile.config.ts`, SKIP_SEED=1). Then: confirm all 9 green, finalize the merged evidence report, optional adversarial audit, decide on pushing the Flutter `development` commits (operator-gated).
