...
| **Reliability (green + deterministic gate)** | 🟡 **In progress** | The suite RUNS on a hermetic emulator with the real Cloud Functions and is **non-circular** (asserts real app/CF output, not values it wrote). It is **not yet 100% green** — currently ~75–90 / 121 and converging now that per-spec isolation removes cross-spec pollution. Until it's green-and-stable in CI it is a *strong test asset*, **not yet a trustworthy regression gate**. |
| **Per-participant / permutation coverage** | ❌ **Representative, not exhaustive (by design)** | The 50 participants are *seeded* (board population/counts). Each variation's journey is *walked once* by a representative token across 3–6 generated paths that together cover 100% of that variation's edges + bounded loops (≤2). The full combinatorial space of "50 distinct users × every loop-count × every move-choice" is **not** enumerated — it is unbounded, so the suite covers every *edge*, not every *path-combination*. |

**Plain English:** every *mechanism* (every transition, every screen action, every CF effect) is tested. It is *not yet reliably green*. And it tests *representative journeys that cover all transitions*, not 50 individually-replayed permutations.
