...
> ✅ **Branch-checked (2026-06-04, stream B):** the mode-engine citations (§7, `participantmode.js`/`participantmetadata.js`) live in the **CF repo on `development`** and the §7g audit is **data-driven** — both current. Cross-checked separately: the on-disk reader `userprofile.component.ts` is `:815` on `production` (was `:814`). Nothing else to re-anchor here.
