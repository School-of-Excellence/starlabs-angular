...
const ID = {
  ...
  LM1: `${TESTRUNID}_LM1`,
  // content_urls (referenced by playlist ads + generalContentUpdate CF; not driven)
  CU1: `${TESTRUNID}_CU1`,
...
async function seedContent() {
  ...
  await db.collection('tier').doc(ID.TIER1).set({ id: ID.TIER1, tier: `TEST_TIER_BASIC_${TESTRUNID}`, ...tag });
  await db.collection('tier').doc(ID.TIER2).set({ id: ID.TIER2, tier: `TEST_TIER_PREM_${TESTRUNID}`, ...tag });
  ...
  //     generalContentUpdate CF stays a no-op). title run-unique.
  ...
  return {
    TESTRUNID, ID, PF, EMAIL, ANALYTICS_PF, BUF_PF,
    counts: {
...
module.exports = { TESTRUNID, ID, PF, EMAIL, ANALYTICS_PF, BUF_PF, ROUTES, SEEDED, seedContent, teardownContent };
