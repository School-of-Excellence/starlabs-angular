...
## Milestone (2026-06-11 cont.)
- **Root sim-contention blocker found+killed** (conflicting session d969021e running the old queue-mobile-walk). After that, the real iterate-to-green began and is FAST (clean ~5-min cycles).
- **journey_flow GREEN ✅** (`3dc1062`): the marquee end-to-end test passes — login → onboarding-locked → intro write (`orientationstatus:initiated`) → booking surface → rich journey (8 attended events, 10 content, projection). 7 non-blank real-screen screenshots in `mobile-evidence/journeyflow/`. "All tests passed!".
- **Bucket iterate-to-green learnings** (apply to all 10): (a) RE-SEED before each run (the app mutates the data; the run cmd already does `node seed-<bucket>.cjs --seed && …`); (b) anti-circular count queries: query ONE field + filter the rest CLIENT-SIDE (no composite index needed); (c) don't navigate into screens with app bugs (e.g. `ScheduleOnboarding.onAppointmentSelect` String-as-int crash) — screenshot the surface instead; (d) pump through long animations (e.g. `AppSettingUpStateV2` ~27s); (e) the `runZonedGuarded` now re-throws genuine `TestFailure` (fail-fast) on all 11 tests; (f) ALWAYS `flutter analyze <test>` after editing — a compile error makes `flutter drive` silently run the LAST build.

## Pending / next (RESUME HERE)
1. **Confirm `journey_flow_test.dart` GREEN** (now compiles). **ROOT CAUSE of all prior contamination FOUND+FIXED:** a SEPARATE 17h headless Claude session (`d969021e`, PID was 59908) was running the OLD queue-mobile-walk (`caffeinate npx playwright test --config=playwright.mobile.config.ts`) on the SAME sim + SAME `breakthroughs-flutter/build/ios` — so `flutter drive --target=journey_flow` kept serving ITS stale `walk_test.dart` app (the `mLAX…-p6 drives N form self-move(s)` output) + concurrent-build errors. Operator approved killing it (2026-06-11). **If sim runs ever show queue/walk output again:** `ps aux | grep -E 'playwright test --config=playwright.mobile|resume d969021e'` and kill it, then `flutter clean` ONCE (clears the stale walk_test app from `build/ios`) before re-running. Don't run two flutter builds against this repo concurrently.
